...
// Vectores de REFERENCIA (PROVISIONALES - reemplazar por IGRF y efemerides del Sol).
// Deben estar en el MISMO marco (p.ej. ECI) y se normalizan dentro de triad().
...
void applyRotation(const float R[3][3], const float v_in[3], float v_out[3]) {
  for (int i = 0; i < 3; i++) {
    v_out[i] = 0;
    for (int j = 0; j < 3; j++) v_out[i] += R[i][j] * v_in[j];
  }
}
...
void readIMU() {
  ...
  Serial.print("Acc: ");
  Serial.print(accelVec[0]); Serial.print("\t");
  Serial.print(accelVec[1]); Serial.print("\t");
  Serial.print(accelVec[2]); Serial.print("\t");
  Serial.print("Gyro: ");
  Serial.print(gyroVec[0]);  Serial.print("\t");
  Serial.print(gyroVec[1]);  Serial.print("\t");
  Serial.print(gyroVec[2]);  Serial.print("\t");
  ...
  switch (mode){
    ...
    case 2:
      if (mag.readMagnetometer(x, y, z)) {
        ...
        Serial.print(x_cal, 2); Serial.print(", ");
        Serial.print(y_cal, 2); Serial.print(", ");
...
void readPhotodiodesRaw() {
  ...
  for (int i = 0; i < 6; i++) pdRaw[i] = analogRead(pdPin[i]);
}
...
void printPhotodiodesRaw() {
  readPhotodiodesRaw();
  for (int i = 0; i < 6; i++) {
    Serial.print(pdRaw[i]); Serial.print(i < 5 ? "," : "\n");
  }
}
...
float readDiodeCalibrated(int i) {
  ...
  if (val < 0) val = 0;   // cara en sombra -> 0
  return val;
}
...
bool computeSunVector(float out_body[3]) {
  ...
  float I[6];
  for (int i = 0; i < 6; i++) I[i] = readDiodeCalibrated(i);
...
void triad(const float b1[3], const float b2[3],
           const float r1[3], const float r2[3],
           float A[3][3]) {

  // Copias normalizadas (no destruimos los originales)
  float u1[3] = { b1[0], b1[1], b1[2] }; normalize3(u1);
  float w2[3] = { b2[0], b2[1], b2[2] }; normalize3(w2);
  float s1[3] = { r1[0], r1[1], r1[2] }; normalize3(s1);
  float s2[3] = { r2[0], r2[1], r2[2] }; normalize3(s2);

  // Triada en el CUERPO
  float t1b[3] = { u1[0], u1[1], u1[2] };
  float t2b[3]; cross3(u1, w2, t2b); normalize3(t2b);
  float t3b[3]; cross3(t1b, t2b, t3b);

  // Triada en REFERENCIA
  float t1r[3] = { s1[0], s1[1], s1[2] };
  float t2r[3]; cross3(s1, s2, t2r); normalize3(t2r);
  float t3r[3]; cross3(t1r, t2r, t3r);
  ...
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++) {
      A[i][j] = 0;
      for (int k = 0; k < 3; k++) A[i][j] += Mb[i][k] * Mr[j][k];
    }
}

// Imprime la actitud como Euler ZYX (yaw, pitch, roll) en grados. Solo para display.
