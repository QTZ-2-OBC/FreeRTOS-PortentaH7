...
HAL_StatusTypeDef I2C_PingPong_IT_Start(I2C_PingPong_t *ctx, I2C_HandleTypeDef *hi2c)
{
...
void I2C_PingPong_IT_OnSlaveRxComplete(I2C_PingPong_t *ctx)
{
    ...
    if (HAL_I2C_Slave_Transmit_IT(ctx->hi2c, ctx->buffer, I2C_PINGPONG_LEN) != HAL_OK)
    {
...
void I2C_PingPong_IT_OnSlaveTxComplete(I2C_PingPong_t *ctx)
{
    ...
    ctx->state         = I2C_PP_STATE_RX_PENDING;
    ctx->watchdogTicks = 0U;
    if (HAL_I2C_Slave_Receive_IT(ctx->hi2c, ctx->buffer, I2C_PINGPONG_LEN) != HAL_OK)
    {
        ctx->state = I2C_PP_STATE_ERROR;
    }
}
