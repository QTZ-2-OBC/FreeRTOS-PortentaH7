...
//
// Response computation happens SYNCHRONOUSLY in ISR context
// (ho_process_command_isr(), called from HAL_I2C_SlaveRxCpltCallback) —
// NOT deferred to HandoverSlave_Routine as originally designed. Confirmed
// on real hardware: the deferred-task approach raced against the master's
// repeated-START into the read phase and lost, even with the handover task
// at osPriorityAboveNormal (above adcs_thread) — A3200 received a stale
// ho_tx_buf[0] value left over from the previous command instead of the
// real response (PREPARE's ACK was spurious; ho_state never actually left
// IDLE). HandoverSlave_Routine now only logs what ho_process_command_isr()
// already decided — see that function's doc comment for the full story.
...
static volatile uint8_t ho_rx_have_len = 0;   /* bytes placed into ho_rx_buf so far   */
...
/* Single-slot queued relay command — see QTZ_HandoverSlave_QueueRelayCommand()'s
 * doc comment. No real ADCS/MILO decision logic exists yet on this
 * prototype; this is just the queue + HO_PROTO_CMD_RELAY_POLL wiring. */
...
/* Number of response bytes to transmit for a given command — most commands
 * respond with 1 byte, except HO_PROTO_CMD_RELAY_POLL's fixed 5-byte
 * [status][subsys][cmd_id][param0][param1] response and
 * HO_PROTO_CMD_HEARTBEAT_REQ's 2-byte [type][rs485_status] response. */
...
/* ── Synchronous command processing (ISR context) ─────────────────────────
 * CONFIRMED ON REAL HARDWARE: computing the response in the deferred task
 * (HandoverSlave_Routine) — even at osPriorityAboveNormal, already above
 * adcs_thread — is NOT fast enough. The master's repeated-START into the
 * read phase can happen before the task ever gets to run, so
 * HAL_I2C_AddrCallback's read-phase branch clocks out whatever was left in
 * ho_tx_buf[0] from the PREVIOUS command, not the real response to the
 * current one. Bench evidence: PING/STATUS_REQ (0 argument bytes) always
 * worked; PREPARE (1 argument byte) never did — A3200 saw a stale ACK
 * left over from the prior PING, while ho_state never actually left IDLE
 * (proven by STATUS_REQ returning NACK three times afterward).
 *
 * Fix: compute the state transition and response byte HERE, synchronously,
 * in ISR context, the instant the full command (cmd + all its argument
 * bytes) has been received — guaranteed to complete before the read-phase
 * AddrCallback can possibly fire. Only logging is deferred to
 * HandoverSlave_Routine (not time-critical, and QTZ_Debug_Log's ISR-safety
 * isn't confirmed) — it re-derives what happened from ho_rx_buf/ho_tx_buf/
 * ho_state, which are already fully decided by the time it wakes.
 *
 * Must stay ISR-safe: plain variable reads/writes only, no blocking calls,
 * no logging. */
static void ho_process_command_isr(void) {
  ...
  switch (cmd) {
    ...
    case HO_PROTO_CMD_HEARTBEAT_REQ:
      ...
      /* byte[1]: this firmware's own RS485 link health, observed passively
       * (see rs485_framed.h's QTZ_RS485F_IsHealthy() doc comment) —
       * informational only on the A3200 side, never a heartbeat failure. */
      ho_tx_buf[1] = QTZ_RS485F_IsHealthy() ? HO_PROTO_RS485_STATUS_OK
                                            : HO_PROTO_RS485_STATUS_FAIL;
      break;
    ...
    case HO_PROTO_CMD_LORA_GRANT:
      ...
      /* DENIED (or anything else): leave ho_lora_tx_pending as-is so the
       * next heartbeat naturally asks again. */
      break;
 ...
 * ISR frames the next HAL transfer and (once the full command is in hand)
 * computes the response synchronously via ho_process_command_isr() above —
 * HandoverSlave_Routine() below now only logs what already happened, it no
 * longer decides ho_tx_buf/ho_state itself. This mirrors the proven bench
 * stub's on_receive/on_request split, adapted to STM32 HAL sequential
 * slave mode, with the response computation moved into the ISR for
 * correctness (see ho_process_command_isr()'s doc comment for why). */
...
void HAL_I2C_SlaveRxCpltCallback(I2C_HandleTypeDef *hi2c) {
  ...
  if (ho_rx_have_len == 0) {
    ...
    if (arg_len > 0U) {
      HAL_I2C_Slave_Seq_Receive_IT(ho_i2c, (uint8_t *)&ho_rx_buf[1],
                                   arg_len, I2C_NEXT_FRAME);
      return;
    }
  ...
  /* Full command now in hand (cmd + all its argument bytes) — compute the
   * response SYNCHRONOUSLY, right here in ISR context, before waking the
   * task. See ho_process_command_isr()'s doc comment for why this can't
   * be left to the deferred task. */
  ho_process_command_isr();
...
/**
 * Logging only — ho_process_command_isr() (called from
 * HAL_I2C_SlaveRxCpltCallback, in ISR context) has already decided
 * ho_tx_buf/ho_state/etc. by the time this task wakes. This task just
 * re-derives what happened from those already-decided values and prints
 * the corresponding log line — it does NOT make any protocol decisions
 * itself anymore (moved to ho_process_command_isr() for correctness, see
...
void HandoverSlave_Routine(void *argument) {
  ...
  for (;;) {
    ...
    uint8_t resp0  = ho_tx_buf[0]; /* already decided by ho_process_command_isr() */
    ...
    switch (cmd) {
      ...
      case HO_PROTO_CMD_PREPARE:
        if (resp0 == HO_PROTO_RESP_BUSY) {
          QTZ_Debug_Warning("[HO] PREPARE rejected - not IDLE\n");
        } else {
      ...
      case HO_PROTO_CMD_START:
        if (resp0 == HO_PROTO_RESP_NACK) {
          QTZ_Debug_Warning("[HO] START rejected - not PREPARING\n");
        } else {
      ...
      case HO_PROTO_CMD_HEARTBEAT_REQ: {
        const char *rs485_str = (ho_tx_buf[1] == HO_PROTO_RS485_STATUS_OK)
                                     ? "OK" : "FAIL";
        if (resp0 == HO_PROTO_RESP_HEARTBEAT_LORA_PENDING) {
          QTZ_Debug_Log("[HO] HEARTBEAT_REQ -> HEARTBEAT_LORA_PENDING (rs485=%s)\n",
                       rs485_str);
        } else {
          QTZ_Debug_Log("[HO] HEARTBEAT_REQ -> HEARTBEAT (rs485=%s)\n", rs485_str);
        }
        break;
      }
      ...
      case HO_PROTO_CMD_LORA_GRANT:
         ...
         * SAMD21-vs-direct-SPI topology question for PLD2). */
        if (subcmd == HO_PROTO_RESP_LORA_APPROVED) {
      ...
      case HO_PROTO_CMD_PAYLOAD_RELAY:
        ...
        QTZ_Debug_Log(
            "[HO] PAYLOAD_RELAY subcmd=0x%02X arg0=%u arg1=%u -> ACK "
            "(would forward)\n",
            subcmd, arg0, arg1);
        break;

      case HO_PROTO_CMD_RELAY_POLL:
        if (resp0 == HO_PROTO_RESP_RELAY_PENDING) {
          QTZ_Debug_Log("[HO] RELAY_POLL -> PENDING subsys=0x%02X cmd_id=0x%02X "
                       "param0=%u param1=%u\n", ho_tx_buf[1], ho_tx_buf[2],
                       ho_tx_buf[3], ho_tx_buf[4]);
        }
        /* NACK case: no log, matches original behavior (silent — this is
         * the expected common case, polled every ~1s). */
        break;
