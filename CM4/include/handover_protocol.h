 ...
 * bytes, and HO_PROTO_CMD_HEARTBEAT_REQ, which reads back 2 bytes (see
 * HO_PROTO_RS485_STATUS_* below) (the A3200 side never requests the richer
 * multi-byte status blob from HO_PROTO_CMD_STATUS_REQ, which still responds
 * with just 1 byte).
 */
...
#define HO_PROTO_CMD_HEARTBEAT_REQ  0xA6U   /* no arg, 2-byte response — see
                                              * HO_PROTO_RS485_STATUS_* below      */
/**
...
/**
 * HO_PROTO_CMD_HEARTBEAT_REQ response byte[1] — this firmware's own local
 * RS485 link health (see CM4/rs485_framed.h's QTZ_RS485F_IsHealthy()),
 * observed passively from whatever RS485 traffic already exists
 * (ADCS_Routine's periodic ping) — no RS485 traffic generated just to
 * produce this byte. Purely informational on the A3200 side: it must never
 * be treated as a heartbeat failure (see handover.h's matching constants
 * and doc comment) — a NOT-OK value still means "H7 answered the
 * heartbeat," just "H7's own RS485 link needs attention."
 */
#define HO_PROTO_RS485_STATUS_FAIL  0x00U
#define HO_PROTO_RS485_STATUS_OK    0x01U
